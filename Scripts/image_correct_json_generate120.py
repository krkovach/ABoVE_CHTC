...
images= glob.glob(os.path.join(images_dir,"*rfl*"))
images = list(filter(lambda x: not x.endswith('.hdr'), images))
images.sort()
config_dict["input_files"] = images

config_dict["anc_files"] = {}
anc_files = glob.glob(os.path.join(images_dir,"*ort_new*"))
anc_files = list(filter(lambda x: not x.endswith('.hdr'), anc_files))
anc_files.sort()
